...
const TextLayer: React.FC = () => {
  const { activeDocumentId, annotations, updateTextElement, deleteTextElement, updateTextElementPosition } = useAppStore();
  ...
  const handlePointerDown = (e: React.PointerEvent, el: TextElement) => {
    if (e.target !== textareaRefs.current[el.id]) {
      setDraggingEl(el.id);
      ...
      {textElements.map(el => (
        <div 
          ...
          style={{
            ...
            minWidth: '200px',
            minHeight: '100px',
            borderRadius: el.type === 'sticky' ? '4px' : '4px',
            border: el.type === 'text' ? '1px dashed #ccc' : 'none',
            cursor: draggingEl === el.id ? 'grabbing' : 'grab'
          }}
        >
          {/* Drag Handle Top Bar */}
          {el.type === 'sticky' && (
            <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: '24px', backgroundColor: 'rgba(0,0,0,0.05)', borderTopLeftRadius: '4px', borderTopRightRadius: '4px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          ...
          <button 
            ...
            style={{ position: 'absolute', top: '-10px', right: '-10px', width: '24px', height: '24px', borderRadius: '50%', backgroundColor: '#ff3b30', border: 'none', color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', zIndex: 10,  boxShadow: '0 2px 4px rgba(0,0,0,0.2)' }}
            title="Delete Note"
          >
            ✕
          </button>
          ...
          <textarea
            ...
            style={{
              ...
              lineHeight: 1.5, overflow: 'auto', minHeight: '60px'
            }}
          />
        </div>
      ))}
